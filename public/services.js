...
services.factory('businessService', ['$http',
  function($http) {
    return {
      ...
      getBusinessesByKeyword: function(keyword, position) {
        ...
        return $http({
          method: 'GET',
          url: '/api/business/keyword/' + keyword + (coordinates ? '?coordinates=' + coordinates : '')
        });
      },
      getNearestBusinesses: function(position) {
        ...
        return $http({
          method: 'GET',
          url: '/api/business/nearest/' + coordinates
        });
      },
  ...
  function($http) {
    return {
      ...
      getLastVisitedBusinessesByUserId: function(id) {
        return $http({
          method: 'GET',
          url: '/api/users/last-visited-businesses/' + id
        });
      },
      getFavoriteBusinessesByUserId: function(id) {
        return $http({
          method: 'GET',
          url: '/api/users/favorite-businesses/' + id
        });
      },
      addBusinessToFavorites: function(userId, businessId) {
        var userData = { favoriteBusinessId : businessId };
        return $http({
          method: 'PUT',
...
services.factory('reviewService', ['$http',
  function($http) {
    return {
...
services.factory('geoLocationService', ['$rootScope',
  function($rootScope) {
    return {
